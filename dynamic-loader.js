...
(function() {
    ...
    function renderArticles() {
        ...
        if (pageArticles.length === 0) {
            ...
            grid.innerHTML = pageArticles.map(a => {
                ...
                return `<a href="/articles/${slug}.html" class="article-card">
    ...
    function getIconForArticle(a) {
        ...
        if (title.includes('bitcoin') || title.includes('doge') || title.includes('meme')) return 'ðŸªâ„¢';
        if (title.includes('wallet') || title.includes('security')) return 'ðŸÂ˝â„¢';
        if (title.includes('tax') || title.includes('accounting')) return 'ðŸÅ“Å ';
        if (title.includes('airdrop') || title.includes('farming')) return 'ðŸâ„¢Å½';
        if (title.includes('defi') || title.includes('staking') || title.includes('yield')) return 'ðŸÂ¦';
        if (title.includes('resume') || title.includes('job') || title.includes('career')) return 'ðŸÅ“Å¾';
        if (title.includes('budget') || title.includes('save') || title.includes('finance')) return 'ðŸâ„¢°';
        if (title.includes('productivity') || title.includes('habit') || title.includes('system')) return 'âš¡';
        if (title.includes('remote') || title.includes('work') || title.includes('nomad')) return 'ðŸÅ’Â';
        if (title.includes('nft') || title.includes('gaming') || title.includes('web3')) return 'ðŸÅ½®';
        if (title.includes('trading') || title.includes('strategy') || title.includes('position')) return 'ðŸÅ“Ë†';
        if (title.includes('exchange') || title.includes('cex') || title.includes('dex')) return 'ðŸÂ˝Å¾';
        if (title.includes('guide') || title.includes('beginner') || title.includes('how')) return 'ðŸÅ“â€Å“';
        if (title.includes('predict') || title.includes('forecast') || title.includes('outlook')) return 'ðŸÂ˝®';
        if (title.includes('solana') || title.includes('sol')) return 'ðŸÅ’â‚¬';
        if (title.includes('risk') || title.includes('scam') || title.includes('protect')) return 'ðŸâ€º¡';
        return 'ðŸÅ“Â˝';
    }
