#!/usr/bin/env node
/**
 * url-parity.mjs — GATE L2 (URL/slug parity on every main deploy)
 * CI-CD-STANDARD.md §5 L2 / Board 294 M8 (Soren §2.3: RWH 326/326, ZB 383/383,
 * RPT 408/408 verified; LSOS 1 %26-vs-& variant; GH Pages has no true 301s → parity-first)
 *
 ...
 *      is flagged (LSOS class) — normalized-match wins, raw-mismatch fails
...
function fileExistsNormalized(dir, target) {
  ...
  const candidates = [
    ...
    target.replace(/\/$/, '') + '/index.html',    // /foo/ → /foo/index.html
...
for (const page of pages) {
  ...
  for (const raw of hrefs) {
    ...
    if (pathPart.includes('${')) continue; // JS template expr — runtime-determined, not statically checkable
    ...
    if (decoded !== target) {
      if (fileExistsNormalized(distDir, decoded) && !fileExistsNormalized(distDir, target)) {
        failures.push(`${rel}: ENCODED-DRIFT href="${raw}" (decodes to ${decoded} which exists — use ${decoded})`);
        continue;
      }
    }
    if (!fileExistsNormalized(distDir, target)) {
      failures.push(`${rel}: DEAD href="${raw}" → ${target} not in dist`);
    }
  }
}
...
if (failures.length) {
  ...
  console.error(`URL-PARITY GATE RESULT: ${failures.length} FAILURE(S) — DEPLOY BLOCKED`);
  process.exit(1);
}
