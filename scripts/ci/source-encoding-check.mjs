#!/usr/bin/env node
/**
 * source-encoding-check.mjs — GATE L2 (source-side encoding integrity)
 * CI-CD-STANDARD.md §5 L2 / Board 294 M6 (Astra §2.8: 211 mojibake files)
 *
 * Asserts (source-side only — never scan dist):
 *   1. No U+FFFD replacement char anywhere in src/ (Astra: '10 Solana Meme Coins Under �—�? ...')
 *   2. No common mojibake sequences (WINDOWS-1252/UTF-8 double-decode artifacts):
 *      â€", â€™, â€˜, â€œ, â€, Ã©, Ã¨, Ã±, Ã¼, Â (in prose contexts)
...
function walk(dir) {
  ...
  catch { return out; } // dir missing → caller decides
...
if (files.length === 0) {
  failures.push(`src dir '${srcDir}' missing or empty — nothing to check`);
} else {
...
if (failures.length) {
  ...
  console.error(`ENCODING GATE RESULT: ${failures.length} FAILURE(S) — DEPLOY BLOCKED`);
  process.exit(1);
}
