#!/usr/bin/env node
/**
 * CANARY.MJS — 7 deploy-blocking assertions (Tailwind v4 pin, BOARD #312)
...
// ---------------- WCAG 2.1 (canonical — vendored from tailwind-specialist verify-contrast-pairs.py) ----------------
function lum(hex) {
...
function check(num, label, pass, detail) {
  ...
  console.log(`${pass ? 'PASS' : 'FAIL'}  canary #${num} — ${label}${detail ? `: ${detail}` : ''}`);
}
...
function main() {
  ...
  console.log(`CANARY — site=${cfg.site} themeClass=${themeClass} repoRoot=${ROOT}\n`);
  ...
  if (tokensOk) {
    ...
    assert(2, 'required token keys', false, 'skipped — tokens.css missing (see #1)');
  }
  ...
  assert(3, `built CSS >= ${cfg.minCssBytes} bytes (dogeking-regression detector)`, sizeOk,
    sizeOk ? `${cssBytes} bytes across ${cssFiles.length} file(s)` : `${cssBytes} bytes — utility build empty?`);
  assert(3, `>= ${cfg.minTokenClasses} token-derived classes in built CSS`, tokenClasses.length >= cfg.minTokenClasses,
    tokenClasses.length ? `found: ${tokenClasses.slice(0, 8).join(', ')}${tokenClasses.length > 8 ? ', …' : ''}` : 'NONE found');
  ...
  if (manifest) {
    ...
    // Note: plain presence-flag is rejected — brand colors are legitimately defined in tokens.css
  ...
  // ---------- #7 reduced-motion block in built CSS (whitespace-insensitive — built CSS is minified) ----------
  ...
  console.log(`\nCANARY RESULT: ${failures === 0 ? 'ALL 7 ASSERTIONS PASS' : `${failures} FAILURE(S) — DEPLOY BLOCKED`} (${cssFiles.length} css, ${htmlFiles.length} html)`);
