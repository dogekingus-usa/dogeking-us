// UNIVERSAL crown theme migration â€˝ handles ALL template types
const fs = require('fs');
...
const NAV_BAR = 
  ...
  '<a href="/" style="font-family:\'Playfair Display\',serif;font-size:1.3rem;font-weight:700;color:#FFD700;display:flex;align-items:center;gap:8px;text-decoration:none;letter-spacing:1px;">ðŸÅ“Ëœ DogeKing</a>' +
...
const FOOTER = 
  ...
  '<span style="color:#606080;font-size:0.8rem;">ðŸÅ“Ëœ 333+ Free Crypto Guides</span>' +
  '<span style="color:#606080;font-size:0.8rem;">â­Â 4.8/5 from 500+ Traders</span>' +
  '</div>' +
  '<p style="color:#404060;font-size:0.75rem;">Â© 2026 DogeKing.us â€˝ Crypto Insights for the Bold. Not financial advice. DYOR.</p>' +
  '</div></footer>';
...
files.forEach(f => {
  ...
  if (html.includes('</footer>') || html.includes('Â© 2026')) {
  ...
  if (html.includes('class="article-card"') && !html.includes('article-card-icon')) {
    ...
    html = html.replace(
      ...
      '<span style="font-size:1.2rem;">ðŸÅ“Ëœ</span>' +
      ...
      '<a href="/crypto-bundle.html" style="margin-left:auto;padding:6px 16px;background:linear-gradient(135deg,#FFD700,#FF8C00);color:#0a0a1a;border-radius:8px;font-size:0.8rem;font-weight:600;text-decoration:none;white-space:nowrap;">Level Up â†â„¢</a>' +
      '</div>\n$1'
    );
  }
