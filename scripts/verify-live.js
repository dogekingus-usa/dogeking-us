// FULL VERIFICATION ‚Äù check every claim against live main domain
...
(async () => {
  ...
  checks.forEach(c => console.log(c[1] ? '  ‚ú‚Ä¶ ' + c[0] : '  ‚ù≈í ' + c[0]));
  ...
  checksP.forEach(c => console.log(c[1] ? '  ‚ú‚Ä¶ ' + c[0] : '  ‚ù≈í ' + c[0]));
  ...
  if (h.includes('gumroad.com/l/crypto-bundle"')) console.log('‚ù≈í MAIN has BROKEN gumroad link (crypto-bundle without prefix)');
  else console.log('‚ú‚Ä¶ MAIN gumroad links are correct');
